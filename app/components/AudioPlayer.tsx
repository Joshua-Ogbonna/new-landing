        ...
        {audioData.map((item, index) => (
          <div key={index} className="text-center">
            <img
              src={item.image}
              alt="Play Music"
              className={`w-full object-cover rounded-lg cursor-pointer transition-transform hover:scale-105 ${
                playingIndex === index ? "border-4 border-[#c2ee03] " : ""
              }`}
         ...
         {audioData.map((item, index) => (
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-300 text-[">{item.title}</p>
              </div>
